...
/* List of tokens {{{*/
 %token ARRAY
  ...
  % token CONSTchar CONSTstring CONSTnum CONSTreal % token < str > ident
/*}}}*/
...
% nonassoc '=' '#' '<' LE '>' GE IN IS
  ...
  % type < typ > Type Expr Factor Designator % type < typlist > ExprList
/*}}}*/
%% Module:MODULE ident ';' ImportList DeclSeq StatBlock END ident '.';

ImportList:
IMPORT ImportListTemp ';' |;

ImportListTemp:
ImportListTemp ',' Import | Import;
...
DataList:
CONST ConstList DataList | TYPE TypeList DataList | VAR VarList DataList |;
...
ProcList:
ProcDecl ';' ProcList | ForwardDecl ';' ProcList |;
...
FPsectionList:
FPsection ';' FPsectionList | FPsection;
...
ConstExprList:
ConstExpr ',' ConstExprList | ConstExpr;
...
StatementSeq:
Statement ';' StatementSeq | Statement;
...
THEN StatementSeq ElseIfBlock Else END | CASE Expr OF CaseList Else END
{
  /*
...
DO StatementSeq END | REPEAT StatementSeq UNTIL Expr
{
...
DO StatementSeq END | FOR ident ASSIGN Expr TO Expr
{
  Entry *smt;
  //$$ = NULL;
    smt = env->lookup (string ($2));
...
DO StatementSeq END
  | LOOP StatementSeq END | WITH GuardStatList Else END | EXIT | RETURN Expr
{
...
CaseLabelList:
CaseLabels | CaseLabels ',' CaseLabelList;

CaseLabels:
ConstExpr | ConstExpr DOTS ConstExpr;

GuardStatList:
Guard DO StatementSeq '|' GuardStatList | Guard DO StatementSeq;
...
|Expr IN Expr | Expr IS Expr
{
  if ($3->node == -1)
  ...
  else
    {
      ...
      if ($3 != basicType[tERROR])
	fprintf (stderr,
		 "Type error at %d: expression does not define a type\n",
		 yylineno);
    }
}
...
|Expr OR Expr
{
  if (CheckEquivalence ($1, basicType[tBOOLEAN])
      && CheckEquivalence ($3, basicType[tBOOLEAN]))
    $$ = basicType[tBOOLEAN];
  else
    {
      $$ = basicType[tERROR];
...
|Expr DIV Expr
{
  if (CheckEquivalence ($1, basicType[tINTEGER])
      && CheckEquivalence ($3, basicType[tINTEGER]))
    $$ = basicType[tINTEGER];
  else
    {
      $$ = basicType[tERROR];
      if ($1 != basicType[tERROR] && $3 != basicType[tERROR])
	fprintf (stderr, "Type error in division DIV at line number %d",
		 yylineno);
    }
}
...
ident
{
  ...
  if (e == NULL)
    {
      fprintf (stderr,
	       "error at line %d: %s is not declared in this scope.\n",
	       yylineno, $1);
...
optSuffix | '^' optSuffix | '(' ExprList ')'
{
  ...
  if (tmp != basicType[tERROR])
	...
	{
	  fprintf (stderr,
		   "error at line %d: Invalid procedure call. not a procedure\n",
		   yylineno);
	  expectedType.pop ();
	  expectedType.push (basicType[tERROR]);
	}
...
{
  string temp[] =
    { "BOOLEAN", "CHAR", "SHORTINT", "INTEGER", "LONGINT", "REAL", "LONGREAL",
"SET", "VOID", "ERROR" };
  TypeSpecifier *t;
  for (int i = 0; i < 10; i++)
    {
